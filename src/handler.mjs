...
import {
  ...
  putLinks,
  deleteLinks,
} from "./controllers/links.controller.mjs";
import {
  getCoverages,
  postCoverages,
  putCoverages,
  deleteCoverages,
} from "./controllers/coverages.controller.mjs";
import {
  deleteDiscounts,
  getDiscounts,
  postDiscounts,
  putDiscounts,
} from "./controllers/discounts.controller.mjs";
...
export const handler = async (event) => {
  ...
  const data =
    ...
    Object.keys(parseJsonToObject(event.body)).length > 0
      ? parseJsonToObject(event.body)
      : {};
  ...
  try {
    if (endpoints.hasOwnProperty(path))
      return await endpoints[path][method.toLowerCase()]({
        id,
        init,
        end,
      });
